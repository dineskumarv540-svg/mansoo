package com.yourquote.app.data.remote
...
class FirebaseRepository {
    ...
    val currentUserEmail: String? get() = auth.currentUser?.email ?: "aarav@yourquote.in"

    // Fetch Posts Stream (with fallback to SampleData for offline/demo)
    ...
    async fun createPost(post: Post): Result<Boolean> {
        return try {
            ...
            // Log & fallback to local simulation
            Result.success(true)
        }
    }
    ...
    async fun toggleSave(postId: String, isSavedCurrently: Boolean): Result<Boolean> {
        return try {
            Result.success(!isSavedCurrently)
        } catch (e: Exception) {
            Result.success(!isSavedCurrently)
        }
    }
    ...
    // Toggle Follow User
    async fun toggleFollowUser(targetUserId: String, isFollowingCurrently: Boolean): Result<Boolean> {
        return try {
            Result.success(!isFollowingCurrently)
        } catch (e: Exception) {
            Result.success(!isFollowingCurrently)
        }
    }
